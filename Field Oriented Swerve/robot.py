...
class MyRobot(wpilib.TimedRobot):
	...
	def swerveDrive(self, x, y, z):
		...
		'''I've been debating adding some checks or completely restructuring to try and make this more efficient,
		but efficiency isn't super important right now and, more importantly, I don't really know python so
		anything I can think of would save minimal time (or possibly even make it take longer).'''
	def brakeMode(self):
		for motor in self.driveMotors:
	...
	def teleopPeriodic(self):
		'''The easiest way to do field oriented is to convert the joystick input before it is passed
		in to the swerve drive function. For example, if the robot was at 0 degrees and the joystick
		pushed forward, it would maintain that forward joystick input and drive the robot forward. If
		the robot was at 90 degrees, it would convert that forward input to be as if the driver had moved
		the joystick left, making the robot go left - which is forward with respect to the field. Z is left
		as is, so all those smooth motions of other drivers turning while going straight forward is the
		result of them twisting the joystick (turning the robot) and pushing the joystick forward in field
		oriented drive (driving the robot further down the field in a straight line, regardless of its position'''
		cos = math.cos(self.navx.getAngle())
		sin = math.sin(self.navx.getAngle())
		x = self.checkDeadband(self.joystick.getX())*cos - self.checkDeadband(self.joystick.getY())*sin
		y = self.checkDeadband(self.joystick.getY())*cos + self.checkDeadband(self.joystick.getX())*sin
